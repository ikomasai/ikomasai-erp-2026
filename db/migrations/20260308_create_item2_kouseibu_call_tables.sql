-- item2: 厚生部呼び出しシステム 初期スキーマ草案
-- 目的:
-- 1. 呼び出し本体 (`item2_calls`)
-- 2. チャットルーム (`item2_chat_rooms`)
-- 3. チャットメッセージ (`item2_chat_messages`)
-- 4. 既読状態 (`item2_chat_read_states`)
-- 5. 監査ログ (`item2_audit_logs`)
--
-- 現時点の仕様では `calls.assigned_to` / `chats.assigned_to` は複数担当を許容するため、
-- 一旦 uuid[] カラムで保持する。
-- 将来的に担当者の履歴や集計要件が増えた場合は中間テーブルへの正規化を検討する。
